...
pipeline {
 ...
 stages {
  ...
  stage("Tests") {
   ...
   steps {
    checkout scm
    snDevOpsArtifact(configurationName:"DevOps-dcvbugbash24pi2sow2.service-now.com-1718947039475",artifactsPayload:"""{"artifacts": [{"name": "devops_scripted_demo.jar","version": "${version}","semanticVersion": "${semanticVersion}","repositoryName": "devops_scripted_demo"}],"stageName": "Tests"}""") 
    snDevOpsPackage(configurationName:"DevOps-dcvbugbash24pi2sow2.service-now.com-1718947039475",name: "sentimentpackage", artifactsPayload: """{"artifacts": [{"name": "devops_scripted_demo.jar","repositoryName": "devops_scripted_demo","version":"${version}"}]}""")
   }
  }
  stage('Deploy'){
   agent any
